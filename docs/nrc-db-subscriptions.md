...
keywords: [NRC, database, subscriptions, notifications, data model, kanalen, abonnementen, filtervalues]
---
...
1. id: Primary Key of table 'kanalen'
...
1. id: Primary Key of table 'abonnementen'
...
This table establishes the N:M relationship between 'abonnementen' and 'kanalen'. The Primary Key (id) of this table is referenced by the 'filtervalues' table. This allows a single subscription to be related to the same 'kanaal' multiple times (which might appear as duplicate information), but with different filter values in the 'filtervalues' table each time.

The key columns are:

1. id: Primary Key of table 'abonnementkanalen'
2. abonnement_id: Foreign Key pointing to an abonnement in the 'abonnementen' table
3. kanaal_id: Foreign Key pointing to a kanaal in the 'kanalen' table

## filtervalues

The 'filtervalues' table corresponds to the 'filters' element within 'abonnement' in the OAS.

Often, it is undesirable to receive all notifications from a kanaal, as this would generate a lot of message traffic. Therefore, it is possible to specify filters on fields from the underlying kanaal in the subscription. Only notifications that match these filter values are sent, thus reducing traffic. The filters are stored in the 'filtervalues' table.

The most important columns are:

1. id: Primary Key of table 'filtervalues'
